...
// Sahifani qabul qilib faylga saqlash
app.post('/upload-html', (req, res) => {
    ...
    fs.writeFile(path.join(__dirname, fileName), html, (err) => {
        if (err) console.log("Fayl saqlanmadi");
    });
    res.json({ success: true });
});

// Admin panel: Fayllar va javob yuborish
    ...
    fs.readdir(__dirname, (err, files) => {
        ...
        res.send(`
            ...
            <form action="/set-message" method="GET">
                <input type="text" name="text" placeholder="Javoblarni yozing...">
                <button type="submit">Yuborish</button>
            </form>
            <p>Xabaringiz: <b>${adminXabari}</b></p><hr>
            <ul>${listItems}</ul>
        `);
    });
});
